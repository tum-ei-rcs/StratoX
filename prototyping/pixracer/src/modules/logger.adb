...
is
   ...
   package body Adapter is
      ...
      begin
         if message'Last < Integer'Last - 1 then
            HIL.UART.write(HIL.Devices.TELE2, HIL.UART.toData_Type ( message & CR & LF ) );
         else
            -- no space left for CR/LF
            HIL.UART.write(HIL.Devices.TELE2, HIL.UART.toData_Type ( message ));
         end if;
      end write;
   end Adapter;
   ...
   begin
      ...
      declare
         ...
         bootstr     : constant String := Bounded_Image.Unsigned8_Img (num_boots);
