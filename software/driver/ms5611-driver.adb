...
--     end Convert;
	...
	begin
		case coeff_id is
			when SENS_T1  => Data_TX(1) := HIL.Byte (CMD_READ_C1);
			when OFF_T1   => Data_TX(1) := HIL.Byte (CMD_READ_C2);
			when TCS      => Data_TX(1) := HIL.Byte (CMD_READ_C3);
			when TCO      => Data_TX(1) := HIL.Byte (CMD_READ_C4);
			when T_REF    => Data_TX(1) := HIL.Byte (CMD_READ_C5);
			when TEMPSENS => Data_TX(1) := HIL.Byte (CMD_READ_C6);
		end case;
                
                transferWithDevice(Device, Data_TX, Data_RX);
      coeff_data := Coefficient_Data_Type( Data_RX(3) ) + Coefficient_Data_Type( Data_RX(2) )*(2**8);
   ...
   is
                Data_TX : constant Data_Array (1 .. 4) := (1 => HIL.Byte (CMD_ADC_READ), others => 0);
		Data_RX : Data_Array (1 .. 4) := (others => 0);
	begin
                transferWithDevice(Device, Data_TX, Data_RX);
      adc_value := Conversion_Data_Type ( Data_RX(4) ) + Conversion_Data_Type ( Data_RX(3) )*(2**8) + Conversion_Data_Type ( Data_RX(2) )*(2**16);
	end read_adc;
   ...
   begin
      ...
      when D1 => 
         case (OSR) is
         when OSR_256  => data(1) := HIL.Byte(CMD_D1_CONV_256);
         when OSR_512  => data(1) := HIL.Byte(CMD_D1_CONV_512);
         when OSR_1024 => data(1) := HIL.Byte(CMD_D1_CONV_1024);
         when OSR_2048 => data(1) := HIL.Byte(CMD_D1_CONV_2048);
         when OSR_4096 => data(1) := HIL.Byte(CMD_D1_CONV_4096);
         end case;
         G_Baro_State.Conv_Info_Pres.OSR := OSR;
       when D2 => 
         case (OSR) is
         when OSR_256  => data(1) := HIL.Byte(CMD_D2_CONV_256);
         when OSR_512  => data(1) := HIL.Byte(CMD_D2_CONV_512);
         when OSR_1024 => data(1) := HIL.Byte(CMD_D2_CONV_1024);
         when OSR_2048 => data(1) := HIL.Byte(CMD_D2_CONV_2048);
         when OSR_4096 => data(1) := HIL.Byte(CMD_D2_CONV_4096);
         end case;
   ...
   begin
	...
	begin
		if TEMP < TEMP_Type (2000.0) then
			...
			if TEMP < TEMP_Type (-1500.0 ) then
				...
				SENS2 := SENS2 + Sense_Type ( 11.0 * (TEMP - TEMP_Type (1500.0) )**2 / 2.0 );
			end if;
		end if;
	...
	is
	begin
		return Pressure_Type ( ( Float ( Pressure_Raw ) * SENS/ Float(2**21) - OFF ) / Float( 2**15 ) );
